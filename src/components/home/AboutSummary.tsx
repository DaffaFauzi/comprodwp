...
export default async function AboutSummary({ lang, cmsData }: { lang: Locale, cmsData?: BootstrapData | null }) {
  const dictionary = await getDictionary(lang)
  const about = dictionary.home.about_section
  
  const section = getCmsSection(cmsData?.sections, 'about_preview')
  const content = (section?.content || {}) as any

  const title = content.title || "PT. Dwi Kusuma Perkasa"
  const desc = content.description || about.desc
  const pillText = content.pill_text || about.pill
  
  const visionTitle = content.vision?.title || about.vision_title
  const visionText = content.vision?.description || about.vision_text
  const missionTitle = content.mission?.title || about.mission_title
  const ctaText = content.primary_cta?.text || about.cta || (lang === 'id' ? 'Pelajari Lebih Lanjut' : 'Learn More')
  
  type MissionItem = { title: string; desc: string }
  
  const missionItems = (content as any).mission?.items?.length ? 
    (content as any).mission.items.reduce((acc: Record<string, MissionItem>, curr: { title: string; description: string }, i: number) => {
      acc[`item_${i}`] = { title: curr.title, desc: curr.description }
      return acc
    }, {})
    : about.mission_items
  ...
  return (
    <section className="bg-[var(--surface)] section-y overflow-hidden">
      <div className="container mx-auto px-[var(--layout-page-px)]">
        ...
        {/* 1. CompRecord<string, unknown> Introduction */}
          ...
          <div className="order-1 lg:order-2">
            <FadeIn direction="left" delay={0.2}>
              <div className="relative aspect-[16/10] rounded-3xl overflow-hidden shadow-xl shadow-slate-200/50 border border-slate-100 group">
                <SafeImage
                  src={getCmsAssetUrl(section?.assets?.[0]?.path) || "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=1200&auto=format&fit=crop"}
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          <FadeIn direction="up" delay={0.2}>
            ...
            <div className="grid gap-6">
              {Object.entries(missionItems as Record<string, MissionItem>).map(([key, item]) => {
